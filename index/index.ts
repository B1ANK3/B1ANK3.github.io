...
const counter = {
    ...
    updateVisitorCounter: () => {
        if (counter.visited.toString() != (counter?.element as HTMLDivElement)?.innerText) {
            if (counter.visited > parseInt((counter?.element as HTMLDivElement)?.innerText)) {
                // counter goes up add animation
            } else {
                // counter goes down
            }
            if (counter.element)
                (counter.element as HTMLDivElement).innerText = counter.visited.toString()
        }
    }
}
